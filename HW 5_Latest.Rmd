...
The evaluation data set is prepared prior.
...
We notice that there are many NAs for `TARGET`. How many are owed to NAs in `STARS`?
...
Every NA in `TARGET` is an effect of an NA in `STARS`. As seen in the training set, the number of sales for wines without an expert rating is overwhelmingly zero. We can add these zeroes in place of the NAs in `TARGET`.
